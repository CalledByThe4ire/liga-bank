...
module.exports = function (task) {
  const {
    paths, store, config, mainBundle,
  } = task;
  const tree = store.tree[mainBundle];
  const { levels } = config.autoCreate;
  const { files } = config.autoCreate;
  ...
  const ignore = {
    ...
    getByExtname(item) {
      ...
      if (['.css', '.styl', '.less', '.scss', '.sass'].includes(extname)) return this.style;

      if (['.html', '.pug', '.jade', '.twig', '.hbs', '.mustache', '.haml', '.erb'].includes(extname)) return this.template;

      if (['.js', '.d.ts', '.ts', '.litcoffee', '.coffee', '.dart', '.clj', '.cljs', '.cljc', '.edn'].includes(extname)) return this.script;
