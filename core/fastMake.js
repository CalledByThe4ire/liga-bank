...
module.exports = {
  ...
  setItems(argv) {
    const items = argv.slice((this.type === 'page' ? 3 : 2));
  ...
  addNode(node, extra) {
    ...
    extra.forEach((item) => {
      ...
      const extname = path.extname(item) || item;
      const name = path.basename(item, extname) || (extname === '.json' ? 'data' : node);
      let content = (config.addContent && config.addContent[extname.slice(1)] || '').replace(/\[name\]/g, name);
      const file = path.join(directory, name + extname);
  ...
  addDirectory(dir) {
    ...
    if (isDirectory(dir)) return this.addMessage(`\x1b[41mFAIL\x1b[0m: Directory "\x1b[36m${where}\x1b[0m" already exist!`);
  ...
  addFile(file, content) {
    ...
    if (isFile(file)) return this.addMessage(`\x1b[41mFAIL\x1b[0m: ${what} "\x1b[36m${where}\x1b[0m" already exist!`);
  ...
  parseArguments(argv, showMessage = true) {
    ...
    if (this.items.length === 0) {
      ...
      try {
        this.items.forEach((item) => {
          ...
          const more = (item.split('[')[1] || '').replace(']', '');
    ...
    if (this.message && showMessage) console.log(`\x1b[1mTry add ${this.type}\x1b[0m:\n${this.sep}\n${this.message}\n${this.sep}\n`);
  },
