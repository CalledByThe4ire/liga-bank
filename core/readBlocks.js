      ...
      fs.readdirSync(paths.blocks(level)).forEach((block) => {
        ...
        if (isFile(json)) {
          try {
            ...
            if (!jsons[block]) jsons[block] = data;
            else jsons[block] = Object.assign(jsons[block], data);
          } catch (e) {
        ...
        if (isFile(use)) {
          ...
          if (data && Array.isArray(data.modules)) {
            data.modules.forEach((obj) => {
              ...
              if (!obj.from || typeof obj.from !== 'string') return (obj.from = path.join(blocks, level, block, 'assets'));
