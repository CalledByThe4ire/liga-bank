...
module.exports = function (type, task) {
  ...
  const {
    store, paths, config, isDev, mainBundle,
  } = task;
  ...
  pages.forEach((page) => {
    ...
    Object.keys(nodes).forEach((node) => {
      ...
      if (BEM.isBlock(node) && deps[node]) checkModules(node, 'import', (store.pages && store.pages[page]), deps, task, importExtnames[type], array);
